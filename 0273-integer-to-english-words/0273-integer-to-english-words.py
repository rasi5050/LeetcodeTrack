class Solution:
    def numberToWords(self, num: int) -> str:
        ...
        def get_two_digit_num(num):
            if not num: return ""
            elif num < 10: return one_digit[num]
            elif num < 20: return two_digit[num]
        ...
        result = ''
        if billion: result = get_three_digit_num(billion) + ' Billion '
        if million: result += get_three_digit_num(million) + ' Million '
        if thousand: result += get_three_digit_num(thousand) + ' Thousand '
        if last_three: result += get_three_digit_num(last_three)
