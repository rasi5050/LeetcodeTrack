class Solution:
    def floodFill(self, image: List[List[int]], sr: int, sc: int, destColor: int) -> List[List[int]]:
        sourceColor=image[sr][sc]
        if sourceColor==destColor: return image
        def dfs(x,y):
            #base conditions
            if x<0 or y<0 or x>len(image)-1 or y>len(image[0])-1 or image[x][y]!=sourceColor:
                return
            image[x][y]=destColor
            dfs(x+1,y)
            dfs(x-1,y)
