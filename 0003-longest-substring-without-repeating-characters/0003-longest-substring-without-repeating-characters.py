class Solution:
    ...
    #Time O(n), earlier it was O(n^2), since The search was within the deque which will take O(n), now its replaced with set(), hence check will take only O(1)==>correction, this is still O(n^2) since inner loop pops element one by one 
